...
这是最容易误解的一处，也是唯一一处**看起来像 bug 的正常行为**：新开一个对话，某些技能没被注入，却已经出现在 AI 的技能目录里。
...
**后果**：一个**已联接**的技能会出现在**每一个**对话的技能目录里 —— 那是 dsh 原生扫描的结果，与注入无关。所以对已联接的技能，**关掉注入不会让模型看不见它**；**要让它默认不出现，请断开联接**（技能页的「不启用」）。反过来，**未联接**的技能只有靠注入才可见。
