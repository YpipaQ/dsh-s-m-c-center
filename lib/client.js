...
`;
		...
		*/
		...
		async function resetSelection(sessionId, cwd) {
			return (await api$1("POST", SMC_API.contextsReset, {
				sessionId,
				cwd
			})).selection.selected;
		}
		...
		*/
		function openPanel(entry) {
			...
			*/
			...
			const render = (skillRows, selection, workspace) => {
				bodyEl.textContent = "";
				const id = sessionId;
				if (id === void 0) {
				...
				(async () => {
					try {
						const [skillBody, selectionBody] = await Promise.all([api$1("GET", SMC_API.skills), sessionId === void 0 ? Promise.resolve({
							selection: { selected: [] },
							workspace: ""
						}) : api$1("POST", SMC_API.contextsGet, {
							sessionId,
							cwd
						})]);
		...
		*/
		...
		var SkillsMcpApi = class {
			...
			* turn off.
			*/
			async resetContext(sessionId, cwd) {
				return await call("POST", SMC_API.contextsReset, {
					sessionId,
					cwd
				});
			}
