...
var SkillsManager = class {
	...
	recoverIndex() {
		...
		for (const slug of names) {
			...
			entries.push({
				...
				linked: this.linkedSource(slug) !== void 0,
	...
	*/
	adopt(sourcePath, kind, source, enabled, releaseDir) {
		const store = this.storeDir();
		...
		this.upsertEntry({
			slug,
			name: parsed.name,
			origin: releaseDir !== void 0 ? join(releaseDir, slug) : sourcePath,
			source,
			enabled,
			linked: enabled,
			adoptedAt: (/* @__PURE__ */ new Date()).toISOString()
	...
	*/
	setSkillEnabled(path, enabled) {
		...
		this.upsertEntry({
			...
			enabled,
			linked: entry?.linked ?? this.linkedSource(managed.slug) !== void 0,
...
function managedFields(store, full) {
	...
	if (!inside(store, resolved)) return {
		managed: false,
		linked: false
	};
	return {
