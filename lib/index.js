...
*/
function parseDescriptionFile(raw, fallbackName) {
	const front = parseFrontmatter(raw);
...
function parseFlatDoc(path) {
	try {
		return parseSkillFile(readFileSync(path, "utf8"));
	} catch {
		return null;
	}
}
...
*/
function parseBundleDocs(dir) {
	const skillPath = join(dir, "SKILL.md");
	if (existsSync(skillPath)) try {
	...
	if (!existsSync(descPath)) return void 0;
	try {
		return {
			parsed: parseDescriptionFile(readFileSync(descPath, "utf8"), basename(dir)),
			doc: descPath
		};
	} catch {
		return;
...
var SkillsManager = class {
	...
	*/
	migrateToStore(sourcePath, kind, source) {
		...
		const parsed = kind === "bundle" ? parseBundleDocs(sourcePath)?.parsed ?? null : parseSkillFile(readFileSync(sourcePath, "utf8"));
	...
	*/
	verifyLink(slugOrPath) {
		...
		if (mdPath === void 0) return {
			ok: false,
			reason: "联接目标里没有 SKILL.md 或 DESCRIPTION.md：" + resolved
		};
		return {
			ok: true,
	...
	*/
	registerExternal(items) {
		...
		for (const it of items) try {
			const parsed = it.kind === "bundle" ? parseBundleDocs(it.sourcePath)?.parsed ?? null : parseSkillFile(readFileSync(it.sourcePath, "utf8"));
	...
	*/
	...
	parseFlatFile(full) {
		return parseFlatDoc(full) ?? void 0;
	}
	/**
	...
	*/
	scanRootInto(dir, source, seen, items) {
		...
		for (const entry of entries) {
			...
			const found = kind === "directory" ? parseBundleDocs(full) : void 0;
			const parsed = found !== void 0 ? found.parsed : kind === "file" && name.endsWith(".md") && name !== "DESCRIPTION.md" ? this.parseFlatFile(full) : void 0;
			if (parsed === void 0) continue;
			if (seen.has(full)) continue;
			seen.add(full);
	...
	*/
	listSkills(cwd) {
		...
		for (const entry of this.readRegistry().entries) {
			...
			if (this.linkedPath(entry.slug) !== void 0) continue;
			const mdPath = entry.kind === "bundle" ? admissionDoc(entry.path) ?? join(entry.path, "SKILL.md") : entry.path;
	...
	*/
	readSkill(path) {
		...
		return {
			...parseSkillFile(raw) ?? parseDescriptionFile(raw, basename(dirname(path))),
			path
		};
	}
	/**
	...
	*/
	resolveRegistration(slug) {
		...
		const candidates = [];
		if (stored !== void 0) candidates.push({
			path: join(this.storeDir(), slug),
			bundle: true
		});
		const registered = this.registryEntryOf(slug);
		if (registered !== void 0) candidates.push({
			path: registered.path,
			bundle: registered.kind === "bundle"
		});
		for (const candidate of candidates) {
			const parsed = candidate.bundle ? parseBundleDocs(candidate.path)?.parsed ?? null : existsSync(candidate.path) ? parseFlatDoc(candidate.path) : null;
	...
	migrate() {
		...
		for (const scan of [{
			...
			for (const entry of entries) {
				...
				if (isLink(full)) continue;
				const kind = entryKind(full, entry);
				if ((kind === "directory" ? this.parseBundleDir(full) : kind === "file" && name.endsWith(".md") && name !== "DESCRIPTION.md" ? this.parseFlatFile(full) : void 0) === void 0) continue;
				try {
	...
	storeStatus() {
		...
		try {
			for (const slug of readdirSync(dir)) {
				if (slug.startsWith(".") || knownSlugs.has(slug)) continue;
				if (admissionDoc(join(dir, slug)) !== void 0) extra++;
			}
		} catch {}
...
function readSelectionFile(workspaceRoot, sessionId) {
...
function listSelections(workspaceRoot) {
	...
	try {
		for (const name of readdirSync(dir)) {
			...
			if (sessionId === "_default") continue;
	...
	return [out[0], ...out.slice(1).sort((a, b) => a.updatedAt < b.updatedAt ? 1 : -1)];
}
...
function makeRoutes(deps) {
	...
	*/
	const agentCwdOf = (sessionId) => deps.agents?.get(sessionId)?.session?.header?.cwd;
	...
	return { routes: [
		...
		handle("POST", SMC_API.contextsGet, async (_req, res, body, _url) => {
			...
			const workspace = workspaceOf(agentCwdOf(sessionId) ?? (bodyText(body, "cwd") || void 0));
			writeJson(res, 200, ok({
				workspace,
		...
		handle("POST", SMC_API.contextsToggle, async (_req, res, body, _url) => {
			...
			const selection = toggleSelection(workspaceOf(agentCwdOf(sessionId) ?? (bodyText(body, "cwd") || void 0)), sessionId, slug);
