...
*/
const SMC_GUIDANCE = "本机已安装 dsh-s-m-c-center 插件（工具管理：技能 / MCP / CLI 管理器）：设置页「Web UI 插件 → 工具管理」。能力：浏览/启用/不启用/删除/导入技能（项目级 .dsh/skills、.agents/skills 与用户级 ~/.dsh/skills、~/.agents/skills）；管理 MCP 服务器（stdio 与 streamable-http，激活/归档）；以及本地 CLI 工具清单与状态（发现 skill 内嵌的 CLI 包装脚本如 scripts/run-cli、以及系统 CLI 如 gh/git/tencent-news-cli，报告是否安装/版本/需更新/子命令/API-Key 状态）。MCP 是真实连接：激活的服务器经 @deepseek-ai/dsh-mcp-client 真正连接并把工具注册为 mcp__<server>__<tool>，激活/归档会实际连接/断开。限制：本插件的数据统一存 ~/.dsh/S-M-C（MCP 激活 mcp.json、归档 mcp-archive.json、CLI 注册表 cli.json；密码/env 明文、权限 0600 由用户自行保证）；用户级技能正本在 ~/.dsh/S-M-C/skills，启用/不启用等于在 skills 目录增删联接（不改写 SKILL.md）；删除为物理删除，不可恢复。用户提到「技能管理 / 技能导入 / MCP 服务器 / MCP 连接 / CLI 工具 / CLI 状态」时即指本插件，请据此协作。";
/**
