 ...
 * - `registry` — user-declared rows in `cli.json`, seeded once with `gh` /
 *   `git` / `tencent-news-cli` on first boot (deletable thereafter).
 *
...
export declare class CliManager {
     ...
     * Deletion sticks all the way down: the built-ins are seeded only on first
     * boot (and persisted then), so deleting the last one leaves an empty
     * document that stays empty instead of resurrecting the defaults.
     */
